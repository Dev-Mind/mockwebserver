 ...
 * in sequence.
 */
public final class MockWebServerExtension implements Closeable, BeforeEachCallback, AfterEachCallback {
    static {
    ...
    @Override
    public synchronized void beforeEach(ExtensionContext extensionContext) throws Exception {
        if (started) return;
        try {
            start();
        }
    ...
    private synchronized void start(InetSocketAddress inetSocketAddress) throws IOException {
        ...
        executor.execute(new NamedRunnable("MockWebServer %s", port) {
            ...
            protected void execute() {
                try {
                    logger.info(MockWebServerExtension.this + " starting to accept connections");
                    acceptConnections();
                }
                catch (Throwable e) {
                    logger.log(Level.WARNING, MockWebServerExtension.this + " failed unexpectedly", e);
                }
            ...
            private void acceptConnections() throws Exception {
                while (true) {
                    ...
                    try {
                        ...
                        logger.info(MockWebServerExtension.this + " done accepting connections: " + e.getMessage());
                        return;
                    }
        ...
        executor.execute(new NamedRunnable("MockWebServer %s", raw.getRemoteSocketAddress()) {
            ...
            protected void execute() {
                try {
                    ...
                    logger.info(
                            MockWebServerExtension.this + " connection from " + raw.getInetAddress() + " failed: " + e);
                }
                catch (Exception e) {
                    logger.log(Level.SEVERE,
                            MockWebServerExtension.this + " connection from " + raw.getInetAddress() + " crashed", e);
                }
            }
            ...
            public void processConnection() throws Exception {
                ...
                if (sequenceNumber == 0) {
                    logger.warning(MockWebServerExtension.this
                                   + " connection from "
                                   + raw.getInetAddress()
            ...
            private boolean processOneRequest(Socket socket, BufferedSource source, BufferedSink sink)
                ...
                if (logger.isLoggable(Level.INFO)) {
                    logger.info(MockWebServerExtension.this + " received request: " + request
